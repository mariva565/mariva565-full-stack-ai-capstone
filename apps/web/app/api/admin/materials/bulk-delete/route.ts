...
export async function POST(request: NextRequest) {
  ...
  await db.delete(materials).where(inArray(materials.id, ids));

  await logActivity(auth.user.sub, "admin_bulk_delete_materials", undefined, {
    count: ids.length,
    ids,
  });

  return NextResponse.json({ message: `Deleted ${ids.length} material(s)` });
}
