...
function SocialIcon({
  ...
  return (
    <a
      href={href}
      aria-label={label}
      className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-indigo-500/10 text-indigo-500 transition-all duration-300 hover:-translate-y-[3px] hover:bg-indigo-500 hover:text-white hover:shadow-[0_5px_15px_rgba(99,102,241,0.3)] dark:text-indigo-400 dark:hover:bg-indigo-500 dark:hover:text-white"
    >
...
export function HowItWorksPage() {
  return (
    <div className="min-h-screen bg-white dark:bg-slate-950">
      ...
      {/* Footer (v1 parity) */}
      <footer className="border-t border-slate-200 bg-slate-50 py-4 text-sm text-slate-500 dark:border-white/10 dark:bg-[#0f172a] dark:text-slate-400">
        <div className="container mx-auto flex flex-col items-center justify-between gap-3 px-4 md:flex-row">
          <p className="mb-0">&copy; 2026 Study Hub. Built with ❤️ for excellence.</p>

          <div className="flex items-center gap-3">
            {/* Contact link */}
            <a
              href="/contact"
              className="inline-flex items-center gap-1 rounded-lg px-3 py-1.5 transition-all hover:bg-indigo-500/10 hover:text-indigo-500 dark:hover:text-indigo-300"
            >
              <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 16 16">
